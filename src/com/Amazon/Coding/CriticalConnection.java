...
public class CriticalConnection {
    ...
    public void dfs(int u,int& t,vector<vector<int>>& g,vector<int>& par,vector<int>& disc,vector<int>& low,vector<pair<int,int>>& criticalConnections){
        disc[u]=low[u]=t++;
        for(int v:g[u]){
            if(v==par[u])continue;
            if(disc[v]==-1){
    ...
    public vector<pair<int,int>> findCriticalConnections(int numOfServers,int numOfConnections,vector<pair<int,int>> connections){
